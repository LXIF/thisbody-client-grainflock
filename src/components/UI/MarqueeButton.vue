    ...
    <button @pointerdown='buttonClick' @pointerup='buttonRelease'>
        <div class='' :class='{"svg-container": !clicked, "svg-container-larger": clicked}'>
            <svg viewBox='0 0 100 100'>
                ...
                <text v-if='!clicked' class='gradient circle-text' :class='{"light": color === "light"}'>
        ...
        <h3 class='gradient'>{{ clicked ? clicktext : text }}</h3>
    </button>
</template>
    ...
    computed: {
        marqueeText() {
            if(!this.clicked) {
                ...
                const marqueeText = this.clicktext.slice(1) + this.clicktext + this.clicktext + this.clicktext.slice(0, 1);
                return marqueeText
            }
        }
    },
...
<style lang="stylus" scoped>
    ...
    .gradient
        background -webkit-linear-gradient(#0F0, #333)
